...
#    def __init__(self, root=r'D:\data\CU_MEF'):
#        self.files = glob(os.path.join(root, 'A','*.*'))
#
#    def __len__(self):
#        return len(self.files)
#    
#    def __getitem__(self, index):
#        A  = torch.from_numpy(np.array(Image.open(self.files[index]))).float().permute(2,0,1)/255.
#        B  = torch.from_numpy(np.array(Image.open(self.files[index].replace('A','B')))).float().permute(2,0,1)/255.
#        GT = torch.from_numpy(np.array(Image.open(self.files[index].replace('A','GT')))).float().permute(2,0,1)/255.
#        
#        return A,B,GT
'''
Others
